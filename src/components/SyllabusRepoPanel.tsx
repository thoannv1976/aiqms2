...
/** Kho đề cương (FULL trang): tải nhiều · trích xuất AI · quản lý. Tự tải khi mở trang. */
export function SyllabusRepoPanel({ onChanged }: { onChanged?: () => void }) {
    ...
    <div className="space-y-4">
      {durable === false && (
        <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 text-xs text-amber-800">
          ...
          khởi động lại/mở rộng. Hãy <b>bật lưu trữ bền vững GCS</b> (xem scripts/setup-gcs.sh / DEPLOY.md mục 7b).
        </div>
      )}
      ...
      <div className="card flex flex-wrap items-end gap-3 p-4">
        ...
        <div className="min-w-72 flex-1">
          <label className="label">Tải lên nhiều đề cương (.docx/.pdf)</label>
          <div className="flex items-center gap-2">
            <input ref={fileRef} type="file" accept=".docx,.pdf" multiple className="input flex-1" />
            <button className="btn-primary whitespace-nowrap" onClick={upload} disabled={busy}>{busy ? "Đang tải…" : "Tải lên kho"}</button>
          </div>
        </div>
      </div>
      ...
      {previewDoc && extracted && (
        <div className="card border-indigo-200 bg-indigo-50/40 p-4">
          ...
          {extracted.clos.length > 0 && (
            <ul className="my-2 max-h-40 overflow-y-auto text-xs text-slate-600">
              {extracted.clos.map((c, i) => <li key={i}>• {c.code}: {c.description}</li>)}
            </ul>
          )}
      ...
      <div className="card p-4">
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button className="btn-outline h-8 py-0 text-xs" disabled={busy} onClick={loadDocs}>🔄 Làm mới</button>
            <button className="btn-outline h-8 py-0 text-xs" disabled={busy} onClick={cleanupMissing}>🧹 Dọn file đã mất</button>
            {docs.length > 0 && (
              <>
        ...
        {docs.length === 0 ? (
          <p className="py-8 text-center text-sm text-slate-400">Chưa có đề cương trong kho cho chương trình này.</p>
        ) : (
              ...
              <tbody>
                {docs.map((d) => (
                  <tr key={d.id} className="border-t border-slate-50 align-top hover:bg-slate-50/50">
                    ...
                    <td className="td">
                      <p className="font-medium text-slate-700" title={d.title}>{d.title}</p>
                      <p className="text-xs text-slate-400" title={d.fileName}>{d.fileName}</p>
                    </td>
                    <td className="td whitespace-nowrap text-slate-500">{fmtSize(d.size)}</td>
                    <td className="td whitespace-nowrap text-slate-500">{new Date(d.createdAt).toLocaleString("vi-VN")}</td>
                    <td className="td">
                      {d.extracted
