...
interface MatrixData { dimension: string; label: string; textMode: boolean; plos: Plo[]; columns: Col[]; cells: Cell[] }
...
export function PloMatrixGrid({ versionId, dimension }: { versionId: string; dimension: string }) {
  ...
  if (data.columns.length === 0) return <div className="card p-6 text-center text-sm text-slate-400">Chưa có cột cho ma trận này (vd PEO–PLO cần có PEO trước).</div>;
        ...
        <table className="min-w-full border-collapse text-sm">
          <thead>
            <tr>
              ...
              {data.columns.map((c) => <th key={c.key} className="px-2 py-1 text-center text-xs font-medium text-slate-500" title={c.label}>{c.label}</th>)}
            </tr>
          </thead>
          <tbody>
            {data.plos.map((p) => (
